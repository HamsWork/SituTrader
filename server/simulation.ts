...
async function simulateDay(ctx: SimDayContext): Promise<SimDayOutput> {
  const {
    ...
    timePriorityMode, today, dayIdx, totalDays,
  } = ctx;
  let { nextSimSignalId, btodExecutedToday } = ctx;
  ...
  return { result: dayResult, nextSimSignalId, btodExecutedToday, shouldBreak: false };
}
