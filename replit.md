...
- **Trade Sync Send Logs:** The `tradesync_logs` DB table records every Trade Sync send attempt (success and failure) with full payload, response, error message, duration, and trade details. Visible via the "Send Logs" tab on the Signal History page (`/signal-history`). API: `GET /api/tradesync/logs?signalId=&success=&limit=`.
...
- **Day-by-Day Simulation:** The Simulate tab (`server/simulation.ts`) replays the full system lifecycle for each trading day in a date range: (1) after-close scan via `detectAllSetups`/`computeQualityScore`, (2) pre-open BTOD ranking (A/B/C, QS≥62, top 15 via `BTOD_TOP_N`), (3) intraday activation via `checkEntryTrigger` with BTOD gate (max 3 executions per day via `BTOD_MAX_TRADES`), (4) magnet touch validation via `validateMagnetTouch`. Uses SSE streaming (`/api/backtest/simulate-stream`) with real-time event push (init/log/progress/day/complete/error/paused/resumed/cancelled/speed events). Frontend connects via `EventSource` with automatic reconnect and catch-up replay. Features a step delay slider (0-5000ms), pause/resume/cancel controls, 4-phase stepper (Pre-Open → Live Monitor → After-Close → EOD), and grouped day-by-day timeline with BTOD/activation/hit detail cards. `simulationRunner.ts` uses a broadcast listener pattern (`addSSEListener`) for zero-latency push to all connected clients. **Monitor BTOD Only** option (default: true) restricts the live monitor phase to only track BTOD Top 3 tickers + already-active signals, skipping non-BTOD on-deck signals. **Pre-fetch optimization**: at the start of each day's live monitor, full-day 1m bars are pre-fetched once per monitored ticker and cached in `ctx.prefetchedBars`; subsequent price lookups use in-memory array instead of per-minute API calls. Activation bars use `fetchIntradayBarsCached` (SQLite cache) during simulation. **Simulation Day Cache**: Per-day results are cached in the `sim_day_cache` PostgreSQL table keyed by (date + config hash). Re-runs with identical parameters skip Polygon API calls and load cached `SimDayResult` + signals snapshot for multi-day continuity. A **Force Run** checkbox (default: unchecked) in the Simulate tab bypasses the cache. Cache can also be cleared via `POST /api/backtest/simulate-cache-clear`.
