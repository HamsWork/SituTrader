...
- **Activation Engine:** Scans intraday bars for entry triggers, managing trade states (ACTIVE/NOT_ACTIVE/INVALIDATED) and advanced stop management (volatility baseline, break-even protection, time stops). Automatic Discord alert dispatch on activation for signals passing profile filter (QS >= 80, allowed setups, 1-per-day gate per instrument type).
...
- **ROI Insights:** Dedicated backtest edge analysis page showing setup rankings with activated win rates and lift, instrument comparison (Shares with real position sizing vs Leveraged ETF using real Polygon LETF daily bars + ibkrOrders conversion where mapping exists, 3x fallback for unmapped tickers vs Options using Black-Scholes ATM estimation with trailing 60-day realized volatility from Polygon daily bars, delta=0.50 vs LETF Options using Black-Scholes ATM on real LETF price with trailing 60-day LETF vol, delta=0.50) with overlay equity curves and per-instrument KPI tabs, recommended strategy P&L simulation (best setup + top tickers + activated), quality score breakdown, top/avoid ticker lists, and 1-trade-per-day selection guide with instrument recommendation. Filterable by setup type.
- **Profit Windows:** Multi-instrument profit windows comparison using REAL Polygon.io data (no synthetic leverage multipliers). Computes P&L for Shares, Leveraged ETFs (real LETF bars), Options (real option premiums, delta=0.50), and LETF Options (compound real LETF bar + real LETF option premium) across 30/60/90-day windows. Only activated backtest trades with ATR-based stops. Over-$1K capital trades flagged. Results cached in `pw_trade_cache`/`pw_cache_meta` DB tables with rebuild controls.
...
**Conclusion:** Split ½ exit strategy outperforms T1-only by ~36% ($189,839 vs $139,652) across 81,906 resolved trades (all backtests + signals). For activated-only trades (74,177), Split ½ wins by $61,334 ($84,335 vs $23,001). The halfway hit rate is 63.8%. Split ½ is the recommended exit strategy.
