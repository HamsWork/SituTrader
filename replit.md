...
- **ROI Insights:** Dedicated backtest edge analysis page showing setup rankings with activated win rates and lift, instrument comparison (Shares 1x baseline vs Leveraged ETF 3x leverage vs Options ~5x leverage — all with proportional stop losses) with overlay equity curves and per-instrument KPI tabs, recommended strategy P&L simulation (best setup + top tickers + activated), quality score breakdown, top/avoid ticker lists, and 1-trade-per-day selection guide with instrument recommendation. Filterable by setup type.
