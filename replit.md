...
- **Trade Sync Integration:** Primary execution path for BTOD trades. When a signal activates, SITU GOAT sends the trade to the Trade Sync API (`server/lib/tradesync.ts`), which handles Discord alerts, IBKR order execution, auto-tracking (target hits, stop loss management), and trade lifecycle updates. Falls back to direct Discord/IBKR if Trade Sync is unavailable. Trades managed by Trade Sync are identified by `tradesyncSignalId` on the `ibkr_trades` table and are skipped by the local monitor. The Trade History page (`/trade-history`) shows trade history from Trade Sync API. The Discord Templates page (`/discord-templates`) displays embed template previews by instrument category.
