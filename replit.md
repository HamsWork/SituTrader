...
- **Performance 2 (Split-Exit Model):** Two-target exit simulation page comparing 50/50 split (50% exits at T1, remaining 50% rides to T2 or exits at break-even) vs the T1-only model. Features comparison banner (T1-Only vs Split-Exit P&L, delta %, T2 hit rate), dual equity curve overlay, trade history with T2 columns (T2 Price, T2 Hit, T1 Leg P&L, T2 Leg P&L), and T2 hit detection using IBKR tpHitLevel for live trades and MFE (Maximum Favorable Excursion) for backtests. Dashboard active trade cards also display T2 target (cyan diamond on progress bar, T2 price in footer/info strip).
